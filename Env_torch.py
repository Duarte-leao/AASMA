...
class CleaningEnv(gym.Env):
    ...
    def __init__(self, grid_size, num_robots, max_episode_steps):
        ...
        self.robot_positions = torch.zeros((num_robots, 2), dtype=int)
        self.viewer = None
        self.dirt_removed1 = torch.zeros(num_robots)
        self.dirt_removed2 = torch.zeros(num_robots)
        self.num_dirt1 = 0
        self.num_dirt2 = 0
        self.min1 = torch.zeros(num_robots)
        self.min2 = torch.zeros(num_robots)
    ...
    def reset(self):
        ...
        self.robot_positions = torch.zeros((self.num_robots, 2), dtype=int)
        ...
        for i in range(self.num_robots):
            row = torch.random.randint(self.grid_size)
            col = torch.random.randint(self.grid_size)
    ...
    def step(self, actions):
        ...
        for i in range(self.num_robots):
            ...
            # elif action == 5:  # Clean
            if torch.any(torch.all(torch.eq(robot_pos, self.dirt_positions1), axis=1)) :
                indices = torch.all(self.dirt_positions1 == robot_pos, dim=1).nonzero(as_tuple=False).squeeze()
                self.dirt_positions1 = torch.cat((self.dirt_positions1[0:indices], self.dirt_positions1[indices+1:]), dim=0)
                self.dirt_removed1[i] +=1
                self.num_dirt1 = self.num_dirt1 - 1
            if torch.any(torch.all(torch.eq(robot_pos, self.dirt_positions2), axis=1)) :

                self.same_position_robots = torch.where(torch.all(self.robot_positions == robot_pos, axis=1))[0]
            
                if len(self.same_position_robots) > 1:
                    count = 0
                    for i in range (len(self.same_position_robots)):

                        if torch.all(actions[int(self.same_position_robots[i])] == 5):
                            count +=1
                            
                        if count == 2 : 
                            indices = torch.all(self.dirt_positions2 == robot_pos, dim=1).nonzero(as_tuple=False).squeeze()
                            self.dirt_positions2 = torch.cat((self.dirt_positions2[0:indices], self.dirt_positions2[indices+1:]), dim=0)
    ...
    def _calculate_reward(self):
        ...
        for i in range(1):
            ...
            # elif torch.any(np.all(np.equal(self.robot_positions[i], self.dirt_positions1), axis=1)) :                         # Robot in a dirt level 1
            #     reward = reward +1
    ...
    def get_observation(self):
        ...
        observation = torch.zeros((self.num_robots + self.num_dirt1 + self.num_dirt2, 2 ), dtype=int)  
        for i in range(self.num_robots):
    ...
    def generate_dirt(self):
        ...
        self.num_dirt1 = torch.randint(torch.pow(self.grid_size,2)*0.1, torch.pow(self.grid_size,2)*0.15)  # Randomly select the number of dirt squares
        # self.num_dirt2 = torch.random.randint(pow(self.grid_size,2)*0., pow(self.grid_size,2)*0.06)
        self.num_dirt2 = 1
        dirt_positions_level1 = torch.empty((self.num_dirt1,2), dtype=int)
        dirt_positions_level2 = torch.empty((self.num_dirt2,2), dtype=int)

        for i in range(self.num_dirt1):
            row = torch.randint(0, self.grid_size - 1)
            col = torch.randint(0, self.grid_size - 1)
            while torch.any(torch.all(torch.equal(dirt_positions_level1, [row, col]), axis=1)) :
                row = torch.randint(0, self.grid_size - 1)
                col = torch.randint(0, self.grid_size - 1)
        ...
        for i in range(self.num_dirt2):
            ...
            while torch.any(torch.all(torch.eq(dirt_positions_level1, [row, col]), axis=1)) or torch.any(torch.all(torch.eq(dirt_positions_level2, [row, col]), axis=1)):
                row = torch.randint(0, self.grid_size - 1)
                col = torch.randint(0, self.grid_size - 1)
    ...
    def _get_render_image(self):
        ...
        img = torch.zeros((self.grid_size * scale, self.grid_size * scale, 3), dtype=torch.uint8)
